...
def record_loop(
    ...
    no_action_count = 0
    timestamp = 0
    step_mode = True  # Flag to indicate if we're in step-by-step mode
    ...
    while timestamp < control_time_s:
        ...
        # Check if we should enter step-by-step mode (only when using policy)
        if policy is not None and events.get("step_policy"):
            step_mode = True
            events["step_policy"] = False
            print("Entering step-by-step mode. Press 'n' to execute next policy action.")
        
        # If in step mode and using policy, wait for 'n' key press
        if step_mode and policy is not None:
            print("Waiting for 'n' key press to execute next action...")
            while not events.get("step_policy") and not events.get("exit_early"):
                time.sleep(0.01)  # Small sleep to avoid busy waiting
                if events.get("exit_early"):
                    break
            
            if events.get("step_policy"):
                events["step_policy"] = False
                print("Executing next policy action...")
